...
def insertNewUser(userId, guildId, name):
    newUser = {"id": userId, "guild_id": guildId, "name": name, "messages": 0, "level": 0, "is_upgraded": 0, "currency": 0}
    c.execute("INSERT INTO users VALUES (:id, :guild_id, :name, :messages, :level, :is_upgraded, :currency)", newUser)
    conn.commit()
    return newUser

def updateUser(user):
    c.execute("UPDATE users SET name = :name, messages = :messages, level = :level, currency = :currency WHERE id = :id AND guild_id = :guild_id", user)
    conn.commit()
